...
export default function RoomCard({ limit }) {
  const settings = {
    dots: true,
    infinite: true,
    speed: 500,
  ...
  const roomShow = limit ? roomData.slice(0, limit) : roomData;
  return (
    <>
      {/* grid and rooms mapin */}
      <div className="grid gap-8 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
        {roomShow.map((room, idx) => (
          <div
            key={idx}
            ...
            <div className="w-full aspect-[4/3] md:aspect-[16/9] relative">
              <Slider {...settings}>
                {room.images.map((img, i) => (
                  <div key={i} className="relative w-full h-64 md:h-80 lg:h-64">
                    <Image
                      src={img}
                      alt={room.name}
                      fill
